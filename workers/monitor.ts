...
export async function checkServices() {
  const services = await db
    ...
    .where(or(lte(stateTable.nextCheckAt, new Date()), isNull(stateTable.serviceId)));
  logger.debugLow(
    'checking services',
    services.map(({ id, name, state }) => ({ id, name, lastCheck: state?.updatedAt, nextCheck: state?.nextCheckAt }))
  );
  await concurrently(services, checkService, config.monitorConcurrency);
}
...
export function start(): void {
  ...
  messageClient.subscribe({ kind: 'action', value: 'test-service' }, ({ id }) => checkServiceById(id));
}
