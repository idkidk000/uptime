...
import { Icon, type IconVariant, iconVariants } from '@/components/icon';

const cache = new Map<IconVariant, Blob>();

export async function GET(_request: NextRequest, { params }: { params: Promise<{ variant: IconVariant | string }> }) {
  const { variant: raw } = await params;
  const variant = iconVariants.includes(raw as IconVariant) ? (raw as IconVariant) : iconVariants[0];
